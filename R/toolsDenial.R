...
#'     setNames(c("number", "dyn_ini", "name","lb_log10value_ub","value", "fitted","prior"))  %>% {.}
#'     within(pars_fitted,{
#'       # split ub lb value ub
#'       mysplit <- str_split(lb_log10value_ub, " ", simplify = T) %>% apply(1, .%>% as.numeric() %>% .[(!is.na(.))] %>% matrix(ncol = 3)) %>% t %>% `colnames<-`(c("lb", "log10value", "ub"))
#'       lb <- mysplit[,1]
#'       ub <- mysplit[,3]
...
#'
#'       # split up value
#'       mysplit <- str_split(value, " ", simplify = T)%>% apply(1, . %>% as.numeric() %>% .[(!is.na(.))] %>% matrix(ncol = 2)) %>% t
#'       value <- mysplit[,2]
...
'a', 1, 1, 0.62575792323431,
'b', 1, 2, 1.39519718933688,
'c', 2, 3, 0.547561881387056,
'd', 2, 4, 0.464598500783588,
'e', 3, 5, -0.112986695702252,
'f', 3, 6, -0.140852993277365)
