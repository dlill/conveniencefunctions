...
initialze_isobole <- function(f, objval, gridmax) {
  ...
  x1 <- uniroot(f1, c(0,gridmax[1]))
  f2 <- function(x2) f(c(0,x2)) - objval
  x2 <- uniroot(f2, c(0,gridmax[2]))
  COUNT <<- COUNT + x1$iter
  COUNT <<- COUNT + x2$iter
...
get_midpoint <- function(p1,p2) {
  ...
  ym <- f(pm)
  interval_x1 <- if(ym < objval) c(pm[1], gridmax[1]) else c(0,pm[1])
  px <- uniroot(fm, interval_x1)
  COUNT <<- COUNT + px$iter
...
p0 <- p[[1]]
p1 <- p[[2]]
p0.5 <- get_midpoint(p0,p1)
p0.25 <- get_midpoint(p0,p0.5)
p0.125 <- get_midpoint(p0,p0.25)
...
j <- (seq(2,length(jmid),2))[[1]]
  for (j in seq(2,length(jmid),2)) {
    iso[[jmid[j]]] <- get_midpoint(iso[[jmid[j-1]]],iso[[jmid[j+1]]])
  }
}
