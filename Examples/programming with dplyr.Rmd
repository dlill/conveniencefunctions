...
This is just for me to play around within the code. The original is from the authors of dplyr.
...
# Good trick: wrap a call in quo() to see what the call is
...
myfun_2 <- function(df, transformation) {
  df %>% 
    gather("key","value", x,y,z) %>% 
    mutate("value" := !!(rlang::enquo(transformation)))
}
