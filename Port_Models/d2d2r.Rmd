...
```{r setup files}
model_def_file <- data_def_files <- readme_file <- NULL
...
# 3. Build the odemodel
myodemodel <-  odemodel(f, modelname = "x")
...
```{r g e and p}
# i <- 1
frame <- map(seq_along(data_defs), function(i) {
...
pars_in_data <- names(covariates(data)) %>% intersect(pinner)
random <- union(get_random(data_def), get_random(model_def))

# linear_pars Would be nice to have a function to automatically detect linear parameters

trafo <- getParameters(g,x,e) %>% 
  setNames(.,.) %>% 
