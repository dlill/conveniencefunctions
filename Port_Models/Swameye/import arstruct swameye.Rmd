...
forcings <-
  fu %>% str_replace_all(c("^.*t, " = "", "\\)" = "")) %>% str_split(", ", simplify = T) %>% matrix(nrow = 2) %>% t %>% 
    as.data.frame(stringsAsFactors = F) %>% `names<-`(c("time", "value")) %>% mutate(time = as.numeric(time)) %>% cbind(name = u, stringsAsFactors = T)
...
init_eqns_init_removed <- pareqns_wo_init[init_ind]

parnames <- c(parnames_wo_init, init_names)
pareqns <- c(pareqns_wo_init, init_eqns_init_removed)
...
```{r}
# Extract data from condlist and supply covtable
...
## forc_pars %>% mutate(val = 10^val)
# forcings <- forcings %>% left_join(forc_pars) %>% select(-value) %>% rename(value = val) %>% mutate(value = 10^value)
# Make a dense forcing frame # Inputgedöns
forcspline <- data$pnas_data_original %>% 
  filter(name == "pEpoR_au") %>% 
  with({smooth.spline(time,value, df = length(time)/2) %>% predict(seq(min(time), max(time), length.out = 200))})
forcings <- forcspline %>% setNames(c("time", "value")) %>% as.data.frame() %>% cbind(name = "pEpoR", stringsAsFactors = T) 

reactions$rates <- reactions$rates %>% replaceSymbols("pEpoR", "pEpoR*scale_pEpoR",.) # Inputgedöns
...
f <- odemodel(reactions, forcings = levels(forcings$name), estimate = estimate, modelname = "ar_x")#, fcontrol = "einspline")
x <- Xs(f, forcings = forcings)

trafo <- map(condlist, "trafo") %>% 
  map(c, c(scale_pEpoR = "exp(scale_pEpoR*log(10))")) %>% # Inputgedöns
  `attr<-`("condition.grid", covtable) %>% `attr<-`("names", condnames)
p <- P(trafo, modelname = "ar_p")
pars <- c(pars, scale_pEpoR = -1) # Inputgedöns
...
model <- dMod.frame("1", g,x,p,data,e, reactions = list(reactions)) %>% 
  appendObj(pars = list(pars))

```

```{r}
checkout_hypothesis(model, 1, "wup")
```

```{r}
# qplot(time, value, data= forcings)
```
...
```{r}
g((x*p)(wuptimes[1:10], pars)[[1]], p(pars)[[1]])
```

```{r}
trafo[[1]] %>% as.eqnvec()
getParameters(p) %>% compare(names(pars))
```

```{r}
bla <- model %>% 
  mutate(fits = list(mstrust(obj, pars, fits = 50, iterlim = 100, cores = 4, parupper = 3, parlower = -3)))#, fixed = c("scale_pEpoR" = 1))))
```
