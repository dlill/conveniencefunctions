...
# Find a way to keep init_ in the equations and duplicate the parameters!
...
# frames <- map(seq_along(data_defs)[1], function(i) {
...
# 1.
conditions_to_datashet <- get_conditions(model_def)
conditions_to_datashet_data_def <- get_conditions(data_def)
# 2.
conditions_to_datashet <- conditions_to_datashet %>% replace(names(conditions_to_datashet_data_def), conditions_to_datashet_data_def)
...
which_conditions_to_datasheet <- setdiff(names(conditions_to_datashet), names(data_sheet))
data_sheet <- data.frame(data_sheet, t(conditions_to_datashet[which_conditions_to_datasheet]), dataset = data_files[i], stringsAsFactors = F) 


# Deal with unused data and with conditions supplied by the data sheet which are not declared anywhere
standard_vars <- c("time", "dataset", names(observables), paste0(names(observables), "_std"), which_conditions_to_datasheet)
...
select_cols <- function(undeclared_vars, obs_estimate, cond_estimate) {
  ui <- miniPage(
    ...
    miniContentPanel(
      ...
      selectInput("conditions", "Condition columns", undeclared_vars, cond_estimate, multiple = T)
      )
  )
  ...
  server <- function(input, output, session) {
    ...
    observeEvent(input$done, {
      returnValue <- list(obs = input$observables, cond = input$conditions)
      stopApp(returnValue)
    })
  }
...
data_sheet <- data_sheet[-which(names(data_sheet)%in%cols$obs)]
# define variables which uniquely identify conditions
condition_names <- c("dataset", names(conditions_to_datashet_data_def), cols$cond)
