...
if(intersect(names(conditions_to_datasheet), names(data_sheet)) %>% length() %>% `>`(0)) warning("Conditions are both present in data def and data sheet")
...
obs_estimate  <- undeclared_vars %>% str_subset("(.*_au)|(.*_std)|(.*_rel)|(.*_abs)")
...
select_cols <- function(undeclared_vars, obs_estimate, cond_estimate) {
  ui <- miniPage(
    ...
    miniContentPanel(
      ...
      selectInput("excluded", "Keep in covariates(data) but don't use in condition name.", c(undeclared_vars, names(conditions_to_datasheet)), setdiff(names(conditions_to_datasheet), cond_estimate), multiple = T)
      )
  )
...
```{r load dMod.frame, message=FALSE}
frames <- bachmann <- readDMod.frame("2018_07_10_15_00_bachmann.rds") %>% appendObj()
```

## Test plots

```{r}
bachmann$data %>% map(plotData)
```

```{r}
pdf()
seq_along(bachmann$hypothesis) %>% map(function(i) {
  plotCombined(bachmann, i) + guides(color = FALSE)
})
dev.off()
```

# Loading parameters and so on
...
# Quick d2d2r from results-workspace
```{r}
# library(R.matlab)
# d2dresult <- readMat(c("~/Promotion/Software/d2d/arFramework3/Examples/Bachmann_MSB2011/Results/BestFit/workspace.mat"))
# saveRDS(d2dresult, "workspaceMAT.rds")
...
```{r x}
armodel <- d2dresult$ar[,1,1][["model"]][,1,1]

ind <- which(names(armodel) %in% c("data", "plot", "condition"))

map(armodel[-ind], unlist)
```
