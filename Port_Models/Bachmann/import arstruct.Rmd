...
mymodel[str_detect(names(mymodel), "^s")] %>% map(unlist) %>% name #
...
pouter_names <- mycondition$p  %>% unlist %>% unname %>% str_replace_all("init_", "")

pars <- pouter_values %>% log %>% `/`(log(10)) %>%  setNames(pouter_names)
```
...
```{r}
myframe <- dMod.frame("Bachmann-Datensatz1", g,x,p,data,e) %>% appendObj(pars = list(pars))
```

```{r}
myframe %>% plotCombined()
ggsave("bla.pdf")
```
